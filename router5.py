...
def start_server():
    ...
    while True:
        ...
        try:
            Thread(target=processing_thread, args=(connection, ip, port, forwarding_table_with_range)).start()
        except:
...
def processing_thread(connection, ip, port, forwarding_table_with_range, max_buffer_size=5120):
