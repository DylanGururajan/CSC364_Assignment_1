...
def generate_forwarding_table_with_range(table):
    ...
    for line in table:
        ...
        if line[0] != "0.0.0.0":
            ...
            # 7. Build the new row.
            new_row = [ip_range[0], ip_range[1], line[3]]
...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    socket5 = create_socket("127.0.0.1", 8005)
    socket6 = create_socket("127.0.0.1", 8006)
    ...
    while True:
        ...
        sending_port = None
        for line in forwarding_table_with_range:
            min_ip = line[0]
            max_ip = line[1]
            interface = line[2]
            if min_ip <= destinationIP_int <= max_ip:
                sending_port = interface
                break
        ...
        if sending_port == "8005" and new_ttl > 0:
            ...
            socket5.send(new_packet.encode())
            ...
            socket6.send(new_packet.encode())
